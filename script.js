...
function updateCustomerCount() {
    const countElement = document.getElementById('totalCustomerCount');
    if (countElement) {
...
window.addEventListener('storage', (e) => {
    if (e.key === 'queen_laundry_orders') {
        loadOrders();
        const currentInput = document.getElementById('trackInput').value;
        if (currentInput) {
            searchOrder(currentInput);
        }
    }
});
